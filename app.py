...
def profile(user_id):
    ...
    if request.form:
        ...
        user.image_url = request.form['image_url'] if request.form['image_url'] else '/static/images/default-pic.png'
    ...
    return render_template('users/edit.html', user=user, form=EditForm())
...
def homepage():
    ...
    if g.user:
        ...
        messages = (Message
            ...
            .filter((User.id==g.user.id) | (Follows.user_following_id==g.user.id))
            .order_by(Message.timestamp.desc())
            .limit(100)
            .all()
        )
